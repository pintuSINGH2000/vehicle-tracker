...
L.LatLng.prototype.bearingTo = function (latlng) {
  const fromLat = this.lat;
  const fromLng = this.lng;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setProgress((prevProgress) => {
        ...
        if (newProgress >= routePositions.length) {
          clearInterval(interval);
          setIsMoving(false);
          return routePositions.length;
        }
        const { position, bearing } = routePositions[Math.floor(newProgress)];
        setVehiclePosition(position);
        setBearing(bearing + 235);
        return newProgress;
      });
    }, 600 - speed);

    return () => clearInterval(interval);
  }, [isMoving, routePositions, speed]);

  const handleProgressBarClick = (event) => {
    const progressBar = progressBarRef.current;
    const clickPosition =
      event.clientX - progressBar.getBoundingClientRect().left;
    const clickProgress = (clickPosition / progressBar.clientWidth) * 100;

    setProgress(clickProgress);
    const { position, bearing } = routePositions[Math.floor(clickProgress)];
    setVehiclePosition(position);
    setBearing(bearing);
  };
  ...
  const handleMouseMove = (event) => {
    if (isHovering) {
      ...
      const mousePosition =
        event.clientX - progressBar.getBoundingClientRect().left;
      const hoverProgress =
        (mousePosition / progressBar.clientWidth) * routePositions?.length;
      setProgress(hoverProgress);
      const { position, bearing } = routePositions[Math.floor(hoverProgress)];
      setVehiclePosition(position);
      setBearing(bearing + 235);
    }
  };
    ...
    >
      ...
      {vehiclePosition && (
        <Marker
          ...
          icon={carIcon(bearing, selectedCar)}
          eventHandlers={{
            click: () => setShowRouteModal(true),
          }}
        />
      )}
        ...
        <div className="configure">
          ...
          <div className="preference">
            ...
            >
              Select Car{" "}
              {showCarMenu ? (
                <IoIosArrowUp className="Menuicon" />
              ) : (
              ...
              <div className="carMenu">
                {CARS.map((car, index) => (
                  <div
                    ...
                    className="carItem"
                    onClick={() => {
                      setSelectCar(car.color);
                      setShowCarMenu(false);
                    }}
                    style={{
                      backgroundColor: car.color === selectedCar && "white",
                    }}
                  >
                    <div
                      style={{
                    ...
                    >
                      ...
                      >
                        ...
                        />
                        <g fill="#2D3134">
                          <path d="M301.9 325.3c3-1.7 4.8-5 4.8-9.7v-1.1l-11-1.7 6.2 12.5zM293.1 303.6l11.5 2.5c-2.4-5.7-6.7-11.3-11.9-14.9l.4 12.4zM284.8 299.5l.4-12c-5.2-1.5-9.5-.3-11.9 3l11.5 9zM271.2 296.7v1.1c0 4.7 1.8 9.8 4.8 14.5l6.3-6.2-11.1-9.4zM288.9 314.2l-6.8 5.5c2.1 1.9 4.4 3.5 6.8 4.7 2.4 1.2 4.7 1.9 6.8 2l-6.8-12.2z" />
                        </g>
                        <path
                        ...
                        />
                        <g fill="#2D3134">
                          <path d="M109.8 228.5c3-1.7 4.8-5 4.8-9.7v-1.1l-11-1.7 6.2 12.5zM100.9 206.8l11.5 2.5c-2.4-5.7-6.7-11.3-11.9-14.9l.4 12.4zM92.7 202.7l.4-12c-5.2-1.5-9.5-.3-11.9 3l11.5 9zM79.1 199.9v1.1c0 4.7 1.8 9.8 4.8 14.5l6.3-6.2-11.1-9.4zM96.8 217.5L90 223c2.1 1.9 4.4 3.5 6.8 4.7 2.4 1.2 4.7 1.9 6.8 2l-6.8-12.2z" />
                        </g>
                        <path
                        ...
                        />
                        <g fill="#2D3134">
                          <path d="M140 200v10l10 5v-10zM160 200l-10-5v10l10 5zM150 225l10 5v-10l-10-5zM160 210v10l10 5v-10zM180 210l-10-5v10l10 5zM170 235l10 5v-10l-10-5zM180 220v10l10 5v-10zM200 220l-10-5v10l10 5zM190 245l10 5v-10l-10-5zM200 230v10l10 5v-10zM240 250v10l10 5v-10zM240 240l-10-5v10l10 5zM230 265l10 5v-10l-10-5zM220 240v10l10 5v-10zM220 230l-10-5v10l10 5zM210 255l10 5v-10l-10-5z" />
                        </g>
                        <g>
                          <path fill="#2D3134" d="M218 129l48-24-18-9-47 24z" />
                          <path
                            fill="#E5E5E5"
                            d="M244.1 86.4l-31 15.5c-1.4.7-2.5 1.8-3.2 3.3l-6 12.8 14 7 44-22-6.9-12.9c-.7-1.4-1.9-2.5-3.4-3.1l-1.8-.8c-1.7-.7-3.8-.7-5.7.2z"
                          />
                          <path
                            d="M216.5 105.3l-5-2.2c-.6.6-1.1 1.2-1.4 2l-6 12.8 14 7v-17.3c-.1-.9-.7-1.9-1.6-2.3z"
                            opacity=".11"
                          />
                          <path
                            d="M255.1 90.1c-.4-.7-.9-1.4-1.5-1.9l-32.8 16.4c-1.7.8-2.7 2.6-2.7 4.4v16l44-22-7-12.9z"
                            opacity=".29"
                          />
                          <path fill="#2D3134" d="M254 103l-30 15v-10l26-13z" />
                        </g>
                        <path
            ...
            >
              {route ? route.title : "Select Route"}{" "}
              {showMenu ? (
                <IoIosArrowUp className="Menuicon" />
              ) : (
                <IoIosArrowDown className="Menuicon" />
        ...
        <div className="progressContainer">
          <div className="progress">
            ...
            >
              <div
                style={{
                  width: `${(progress * 100) / routePositions?.length}%`,
                  height: "100%",
                  backgroundColor: "red",
                }}
              />
              <div
                ...
                style={{
                  left: `${(progress * 100) / routePositions?.length}%`,
                }}
              />
            </div>
            {!isMoving && (
            ...
            {isMoving && (
              <div className="playBtn" onClick={handleStop}>
                <FaStop />
              </div>
            )}
            <div className="playBtn" onClick={handleReplay}>
              <MdReplay style={{ fontSize: "20px" }} />
            </div>
          </div>
          ...
          <div className="speedSliderContainer">
            <div
              ...
              style={{ background: speed >= 100 ? "blue" : "#dbd4d4" }}
              className="speedTracker"
            ></div>
            <div
              onClick={() => setSpeed(200)}
              style={{ background: speed >= 200 ? "blue" : "#dbd4d4" }}
              className="speedTracker"
            ></div>
            <div
              onClick={() => setSpeed(300)}
              style={{ background: speed >= 300 ? "blue" : "#dbd4d4" }}
              className="speedTracker"
            ></div>
            <div
              onClick={() => setSpeed(400)}
              style={{ background: speed >= 400 ? "blue" : "#dbd4d4" }}
              className="speedTracker"
            ></div>
            <div
              onClick={() => setSpeed(500)}
              style={{ background: speed >= 500 ? "blue" : "#dbd4d4" }}
              className="speedTracker"
            ></div>
            <div
              className="progressDotIcon"
          ...
          <div className="selected-speed">Speed: {speed}</div>
          <IoMdClose
